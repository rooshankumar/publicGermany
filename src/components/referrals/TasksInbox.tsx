...
export default function TasksInbox() {
  ...
  const Row = ({ t }: any) => (
    <div className="flex items-center gap-2 px-2.5 py-1.5 border-b border-border/50 last:border-0 hover:bg-muted/30">
      <Checkbox
      ...
      <button className="flex-1 text-left min-w-0" onClick={() => navigate(`/editor/referrals/${t.referral_id}`)}>
        <div className={`text-[11px] ${t.status === 'done' ? 'line-through text-muted-foreground' : 'text-card-foreground'} truncate`}>{t.title}</div>
        {t.referrals?.full_name && (
          <div className="text-[9px] text-muted-foreground truncate">{t.referrals.full_name}</div>
        )}
      </button>
      {t.due_date && (
        <span className="inline-flex items-center rounded-full border px-1.5 py-0.5 text-[8px] font-medium text-muted-foreground gap-0.5 shrink-0">
  ...
  const Group = ({ title, items, tone }: any) => (
    <div className="border rounded-lg bg-card overflow-hidden">
      <div className={`px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider border-b border-border/50 ${tone}`}>
