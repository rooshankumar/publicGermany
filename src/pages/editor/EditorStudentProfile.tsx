        ...
        {loading ? (
          ...
          <>
            ...
            <header className="rounded-lg border bg-card">
              <div className="p-2.5 flex items-center gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-sm font-semibold text-card-foreground truncate">{profile?.full_name || 'Student'}</h1>
            ...
            <Tabs defaultValue={defaultTab} className="space-y-2">
              <TabsList className="bg-muted/50 border h-7 p-0.5 rounded-lg gap-0.5">
                {permission.can_view_profile && (
                  <TabsTrigger value="profile" className="rounded-md px-2 text-[10px] h-6 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-foreground text-muted-foreground">
                    <User className="h-3 w-3 mr-1" />Profile
                  </TabsTrigger>
                )}
                {permission.can_view_documents && (
                  <TabsTrigger value="documents" title="Documents" className="rounded-md px-2 text-[10px] h-6 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-foreground text-muted-foreground">
                ...
                {permission.can_view_applications && (
                  <TabsTrigger value="applications" title="Applications" className="rounded-md px-2 text-[10px] h-6 data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-foreground text-muted-foreground">
                      ...
                      {!profile ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-2">
                          {[
                            ...
                            <div key={label as string} className="border-l-2 border-border/50 pl-2 py-0.5">
                              <p className="text-[9px] font-semibold tracking-wider uppercase text-muted-foreground">{label as string}</p>
                              <p className="text-[11px] font-medium mt-0.5 text-card-foreground truncate">{(value as string) || '—'}</p>
                            </div>
                          ))}
                        </div>
                      )}
                          ...
                          <div key={doc.id} className="flex items-center justify-between gap-2 px-2.5 py-2 hover:bg-muted/30 transition-colors">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              ...
                              <div className="min-w-0">
                                <p className="text-[11px] font-medium text-card-foreground truncate">{doc.file_name}</p>
                      ...
                      <div className="divide-y divide-border/50">
                        {applications.map((app: any) => (
                          <div key={app.id} className="flex items-center justify-between gap-2 px-2.5 py-2 hover:bg-muted/30 transition-colors">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              <div className="h-7 w-7 rounded bg-gradient-to-br from-primary/10 to-amber-50 flex items-center justify-center shrink-0">
                                <GraduationCap className="h-3.5 w-3.5 text-primary" />
                              </div>
                              <div className="min-w-0">
                                <p className="text-[11px] font-medium text-card-foreground truncate">{app.university_name}</p>
                                <p className="text-[9px] text-muted-foreground truncate">{app.program_name}</p>
                              </div>
                            </div>
                            <span className="inline-flex items-center rounded-full bg-muted text-muted-foreground px-1.5 py-0.5 text-[8px] font-medium capitalize shrink-0">
                              {app.status}
                            </span>
                          </div>
                        ))}
